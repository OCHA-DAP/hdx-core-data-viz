...
export async function buildBubbleData(
  ...
  const [popRows, conflictRows, foodRows, idpRows, povertyRows, riskRows, fundingRows] =
    await Promise.all([
      ...
      safeRead(partUrl('geography-infrastructure/baseline-population', level), [
        ...scopeCols, codeCol, nameCol, 'gender', 'age_range', 'population', 'reference_period_start',
      ]),
      needsConflict
        ? safeRead(partUrl('coordination-context/conflict-events', level), [
            ...scopeCols, codeCol, nameCol, 'fatalities', 'reference_period_start',
          ])
        : Promise.resolve([]),
      needsFood
        ? safeRead(partUrl('food-security-nutrition-poverty/food-security', level), [
            ...scopeCols, codeCol, 'ipc_phase', 'ipc_type', 'population_fraction_in_phase', 'reference_period_start',
          ])
        : Promise.resolve([]),
      needsIDPs
        ? safeRead(partUrl('affected-people/idps', level), [
            ...scopeCols, codeCol, 'population', 'reference_period_start',
          ])
        : Promise.resolve([]),
      needsPoverty
        ? safeRead(partUrl('food-security-nutrition-poverty/poverty-rate', level), [
            ...scopeCols, codeCol, 'mpi', 'headcount_ratio', 'reference_period_start',
          ])
        : Promise.resolve([]),
      // Always at level 0: risk colors + optional risk score axis
      level === 0
        ? safeRead(`${BASE}/coordination-context/national-risk.parquet`, [
            'location_code', 'risk_class', 'overall_risk', 'reference_period_start',
          ])
        : Promise.resolve([]),
      level === 0 && ids.includes('funding_gap_pct')
        ? safeRead(`${BASE}/coordination-context/funding.parquet`, [
            'location_code', 'requirements_usd', 'funding_usd', 'reference_period_start',
          ])
        : Promise.resolve([]),
    ])
  ...
  function mapForId(id: string): Map<string, number> {
    switch (id) {
      ...
      case 'conflict_fatalities': return conflictMap
      case 'ipc_phase3_fraction':  return foodMap
      case 'idp_population':       return idpMap
      case 'poverty_headcount_ratio': return povertyHeadcountMap
      case 'poverty_mpi':          return povertyMpiMap
      case 'national_risk_overall': return riskScoreMap
      case 'funding_gap_pct':      return fundingGapMap
